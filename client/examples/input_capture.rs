...
async fn main() -> ashpd::Result<()> {
    ...
    let zones_response = input_capture.zones(&session).await?;
    let zones = zones_response.response()?;
    println!("Available Zones: {:?}", zones);

    // Set up barriers on all edges of each zone, even the overlapping ones (if we have multiple
    // zones). Those will fail and that's fine. The alternative is a more sophisticated algorithm
    // to only set up barriers on the outside edges.
    ...
    loop {
        tokio::select! {
            Some(activated) = activated_stream.next() => {
                ...
                input_capture.release(&session, activated.activation_id(), None).await?;
