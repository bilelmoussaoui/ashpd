...
mod imp {
    ...
    impl UsbPage {
        ...
        fn add_device_row(&self, page: &super::UsbPage, device: &(String, UsbDevice)) {
            let row = super::UsbDeviceRow::with_device(
                page.clone(),
                device.0.clone(),
                device.1.writable.unwrap_or_default(),
            );
