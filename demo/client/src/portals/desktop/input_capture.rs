...
mod imp {
    ...
    pub struct InputCapturePage {
        ...
        pub barrier_positions: Arc<Mutex<HashMap<u32, (i32, i32, i32, i32)>>>,
    }
    ...
    async fn start_session(&self) {
        ...
        match self.create_session().await {
            ...
            Err(err) => {
                ...
                self.error("Failed to start input capture session");
    ...
    async fn enable(&self) {
        ...
        let proxy = match spawn_tokio(InputCapture::new()).await {
            ...
            Err(err) => {
                ...
                self.error("Failed to create InputCapture proxy");
                return;
            }
        };
        ...
        if let Some(session) = session_guard.as_ref() {
            match proxy.enable(session).await {
                ...
                Err(err) => {
                    ...
                    self.error("Failed to enable input capture");
                }
            }
        }
    }
        ...
        if let Some(session) = session_guard.as_ref() {
            match proxy.disable(session).await {
                ...
                Err(err) => {
                    ...
                    self.error("Failed to disable input capture");
                }
            }
        }
    }
                ...
                let barrier_base_text = match activated.barrier_id() {
                    Some(ashpd::desktop::input_capture::ActivatedBarrier::Barrier(id)) => {
                        ...
                        if let Some(position) = barrier_positions.get(&id.get()) {
                            barrier_name_from_position(*position)
                        } else {
                            format!("Barrier {}", id)
                        }
                    }
        ...
        if let Some(session) = session_guard.as_ref() {
            match proxy.release(session, activation_id, None).await {
                ...
                Err(err) => {
                    ...
                    self.error("Failed to release input capture");
                }
            }
        }
    }
        ...
        *self.imp().barrier_positions.lock().await = barrier_positions;
        ...
        match eis_result {
            ...
            Err(err) => {
                ...
                self.error("Failed to connect to EIS");
            }
        }
...
/// Calculate the barrier name from its position
fn barrier_name_from_position(position: (i32, i32, i32, i32)) -> String {
...
fn calculate_outside_barriers(
    ...
    Vec<Barrier>,
    std::collections::HashMap<u32, (i32, i32, i32, i32)>,
) {
    ...
    for edge in all_edges.iter().filter(|e| !shared_edges.contains(e)) {
        ...
        if let Some(id) = BarrierID::new(barrier_id) {
            ...
            barrier_positions.insert(barrier_id, position);
